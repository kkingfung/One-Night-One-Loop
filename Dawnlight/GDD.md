...
- **ランダムアップグレード**: ウェーブクリア後に3択でパワーアップ選択
...
### 3.4 ローグライト・ランダムアップグレードシステム

ウェーブクリア後に**3つのランダムアップグレード**から1つを選択。
飽きを防ぎ、毎回異なるビルドが楽しめるシステム。
...
│  ─────────────────────────────────────                          │
│  • 3枚のカードが表示                                            │
│  • レアリティ: Common(50%) > Uncommon(30%) > Rare(15%)          │
│    > Epic(4%) > Legendary(1%)                                   │
│  • ウェーブが進むほどレアが出やすい                              │
│  • 1回だけリロール可能                                          │
│  • スキップ可能（ボーナス経験値）                                │
...
| カテゴリ | 説明 | 例 |
|---------|------|-----|
| **武器** | 武器の切り替えや強化 | 大鎌→杖、攻撃範囲UP |
| **スキル** | 新しいアビリティ解放 | 魂爆発、ダッシュ、シールド |
| **パッシブ** | 常時効果のステータスUP | HP+20%、クリティカル+5% |
| **魂親和** | 特定ソウルタイプの効果増加 | Tiger魂効果2倍 |
| **ビジュアル** | 見た目の変化（ステータスも微増） | 発光オーラ、武器トレイル |
...
| 名前 | 効果 | ビジュアル |
|------|------|-----------|
| 死神のダッシュ | 回避ダッシュ解放 | ダッシュ時に残像 |
| 魂の吸収 | 敵撃破時HP回復 +3% | 緑のオーラ |
| 杖使い | 武器を杖に変更（長射程、速攻撃） | 武器モデル変更 |
| 範囲拡大 | 攻撃範囲 +25% | 武器トレイル拡大 |

**Epic（エピック）- 4%出現**
| 名前 | 効果 | ビジュアル |
|------|------|-----------|
| 魂爆発 | 新スキル: 周囲の敵にダメージ | 紫の爆発エフェクト |
| 二重打 | 30%確率で2回攻撃 | 残像エフェクト |
| 不死身 | 1回だけ致死ダメージで生存 | 金の盾アイコン |
| 延長リーパー | リーパーモード +5秒 | 強化オーラ |

**Legendary（レジェンダリー）- 1%出現**
| 名前 | 効果 | ビジュアル |
|------|------|-----------|
| 死神覚醒 | 全ステータス +30%、常時微発光 | 全身発光エフェクト |
| 魂の支配者 | 全ソウルタイプ効果 2倍 | 虹色オーラ |
| 無限の刃 | 攻撃速度 +50%、クリティカル +20% | 武器が炎を纏う |

#### ソウルセットボーナス

同じソウルタイプを一定数集めるとセットボーナスが発動。

| ソウルタイプ | 3個 | 5個 | 8個 |
|------------|-----|-----|-----|
| **Tiger（虎）** | 攻撃速度 +10% | +20%、クリ率 +5% | +30%、バーサーク（低HP時ダメUP） |
| **Horse（馬）** | 移動速度 +8% | +15%、ダッシュCD -20% | +25%、ダッシュ無敵時間延長 |
| **Dog（犬）** | ソウル持続 +3秒 | +6秒、ドロップ率 +10% | +10秒、死亡時ソウル維持 |
| **Cat（猫）** | 回避率 +5% | +10%、回避時反撃 | +15%、完全回避（3秒CD） |
| **Deer（鹿）** | CD -5% | -10%、スタミナ回復 +20% | -15%、スキル2回連続使用 |
| **Wolf（狼）** | 範囲ダメージ +10% | +20%、貫通攻撃 | +30%、連鎖ダメージ |

#### 敵バリアント（ParagonMinionsカラー活用）

敵の色バリエーションに特殊効果を付与。

| 色 | 敵の特性 | ドロップ |
|----|---------|---------|
| **Default（通常）** | 基本ステータス | Common魂 |
| **Red（赤）** | 攻撃時に炎上（DoT） | Tiger魂確定 |
| **Blue（青）** | 攻撃時に減速 | Horse魂確定 |
| **Green（緑）** | 毒攻撃（持続ダメージ） | Cat魂確定 |
| **Black（黒）** | HP・ダメージ 1.5倍（エリート） | Rare魂確定 |
| **White（白）** | HP 2倍、高防御（タンク） | Golden魂（2倍価値） |

---

### 3.5 永続アップグレードショップ（メタ進行）
...
| **UpgradeSubsystem** | **ランダムアップグレード生成・管理（NEW）** |
| **UpgradeDataAsset** | **アップグレード定義（レアリティ、効果、VFX）** |
| **SoulSetBonusSystem** | **ソウルセットボーナス計算** |
| **EnemyVariantSystem** | **敵カラーバリアント（特殊効果付与）** |
